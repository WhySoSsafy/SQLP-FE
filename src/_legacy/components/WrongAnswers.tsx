...
export default function WrongAnswers({ sessions }: Props) {
  ...
  const items = buildWrongAnswerItems(sessions).map((item) => ({ ...item, done: doneIds.includes(item.id) }));

  const filtered = items.filter((item) => {
    if (filter === "all") return !item.done;
    if (filter === "done") return item.done;
    if (filter === "wrong") return !item.isCorrect && !item.done;
    if (filter === "vague") return item.understanding === "애매" && !item.done;
    if (filter === "unknown") return item.understanding === "모름" && !item.done;
    return true;
  });

  const toggle = (id: string) => {
    setDoneIds((prev) => (prev.includes(id) ? prev.filter((doneId) => doneId !== id) : [...prev, id]));
  };

  const countForFilter = (filterType: FilterType) => {
    if (filterType === "all") return items.filter((i) => !i.done).length;
    if (filterType === "wrong") return items.filter((i) => !i.isCorrect && !i.done).length;
    if (filterType === "vague") return items.filter((i) => i.understanding === "애매" && !i.done).length;
    if (filterType === "unknown") return items.filter((i) => i.understanding === "모름" && !i.done).length;
  ...
  if (sessions.length === 0) {
    return (
      <div style={{ maxWidth: "900px" }}>
        <div style={{ backgroundColor: "#FFFFFF", borderRadius: "12px", padding: "3rem", boxShadow: "0 1px 3px rgba(0,0,0,0.06)", textAlign: "center", color: "#9CA3AF" }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-start", gap: "1rem" }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.625rem", flexWrap: "wrap" }}>
                  ...
                  {item.done && (
                    <span style={{ backgroundColor: "#ECFDF5", color: "#065F46", fontSize: "0.75rem", fontWeight: 600, padding: "0.125rem 0.5rem", borderRadius: "999px" }}>
                      ✓ 복습 완료
                    </span>
                  )}
                </div>
                ...
                {/* Concept */}
                <div style={{ fontSize: "0.875rem", fontWeight: 500, color: "#374151", marginBottom: "0.625rem" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.5rem 1.5rem", marginBottom: "0.75rem" }}>
                  <div style={{ fontSize: "0.8125rem" }}>
                    ...
                    <span style={{ color: "#374151" }}>{item.missed.length === 0 ? "없음" : item.missed.join(", ")}</span>
                  </div>
                  <div style={{ fontSize: "0.8125rem" }}>
                    <span style={{ color: "#6B7280", fontWeight: 500 }}>오개념: </span>
                    <span style={{ color: "#374151" }}>{item.errors.length === 0 ? "없음" : item.errors.join(", ")}</span>
                  </div>
                </div>

                <div style={{ backgroundColor: "#F9FAFB", borderRadius: "8px", padding: "0.75rem", fontSize: "0.8125rem", color: "#374151", lineHeight: 1.6 }}>
              ...
              >
                {item.done ? <CheckCircle size={22} color="#10B981" /> : <Circle size={22} color="#D1D5DB" />}
              </button>
            </div>
          </div>
        ))}
