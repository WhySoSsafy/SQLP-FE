...
export default function LoginPage({ onLogin }: Props) {
  const [email, setEmail] = useState("seun@sqlp.kr");
    ...
    <div style={{ minHeight: "100vh", display: "flex", backgroundColor: "#FAFAF8" }}>
      ...
      >
        <div style={{ width: "100%", maxWidth: "400px" }}>
          ...
          <p style={{ textAlign: "center", fontSize: "0.875rem", color: "#6B7280" }}>
            계정이 없으신가요?{" "}
            <a href="#" style={{ color: "#C8962A", fontWeight: 500, textDecoration: "none" }}>
              회원가입
            </a>
          </p>
        </div>
      ...
      >
        <div style={{ width: "100%", maxWidth: "440px" }}>
          <h2 style={{ color: "#FFFFFF", fontSize: "1.5rem", fontWeight: 600, marginBottom: "0.5rem" }}>
            AI 기반 SQLP 학습의 새로운 기준
          </h2>
          ...
          {[
            {
              icon: <Database size={20} color="#C8962A" />,
              title: "JSON 분석 결과 등록",
              desc: "외부 AI가 분석한 풀이 결과 JSON을 업로드하거나 직접 붙여넣어 학습 기록으로 등록하세요.",
            },
            {
              icon: <Brain size={20} color="#C8962A" />,
              title: "문제별 이해도 진단",
              desc: "문제마다 잘함·애매·모름을 시각화하고, 취약 개념을 자동으로 분류합니다.",
            },
            {
              icon: <BookOpen size={20} color="#C8962A" />,
              title: "맞춤 복습 추천",
              desc: "학습 패턴을 분석하여 오늘 복습해야 할 개념을 자동으로 추천합니다.",
            },
          ].map((card, i) => (
            <div
              key={i}
